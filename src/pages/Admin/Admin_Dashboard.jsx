...
const AdminDashboard = () => {
  ...
  const fetchTicketedEvents = async () => {
    ...
    try {
      const response = await axios.get("http://localhost:5002/admin/ticketed-events", {
      ...
      if (response.data.success) {
        const events = response.data.data
          .filter((event) => event.Tickets && event.Tickets.length > 0) // Only include events with tickets
          .map((event) => ({
            id: event.id,
            name: event.name,
          }));
        ...
        console.error("Failed to fetch ticketed events:", response.data.message);
      }
    } catch (error) {
    ...
    const fetchUpcomingEvents = async () => {
      try {
        ...
        if (response.data.success) {
          ...
          console.error("Failed to fetch upcoming events:", response.data.message);
          ...
          <div className="grid grid-cols-2 gap-6 mt-6">
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                >
                  <h3 className="text-lg font-semibold text-black mb-2">{metric.title}</h3> {/* Add title */}
                  <span className="text-5xl font-bold text-black"> {/* Adjust font size as needed */}
                    {metric.value}
                  </span>
              ...
              <div className="bg-white rounded-full px-4 py-2 mb-4">
                ...
                >
                  <option value="" disabled>
                    {ticketedEvents.length === 0 ? "No events available" : "Select an Event"}
                  </option>
      ...
      >
        {eventStatusData.map((entry, index) => (
          <Cell key={index} fill={entry.color} />
                ...
                <p className="text-white text-center mt-4">No data available for this event.</p>
              )}
            </div>
          </div>
                ...
                <table className="w-full text-left border-collapse">
                  ...
                  <tbody>
                    {recentReservations.map((res, i) => (
                      <tr key={i} className="border-b border-gray-600 text-gray-300">
                        ...
                        <td>{res.Ticket?.ticket_type || "N/A"}</td>
                        <td>
                          {res.ClaimingSlot?.claiming_date
                            ? new Date(res.ClaimingSlot.claiming_date).toLocaleDateString("en-US", {
                                year: "numeric",
                                month: "short",
                                day: "numeric",
                              })
                        ...
                        <td>
                          {res.ClaimingSlot?.start_time
                            ? new Date(`1970-01-01T${res.ClaimingSlot.start_time}Z`).toLocaleTimeString("en-US", {
                                hour: "2-digit",
                                minute: "2-digit",
                                hour12: true,
                              })
                          ...
                          {res.ClaimingSlot?.end_time
                            ? new Date(`1970-01-01T${res.ClaimingSlot.end_time}Z`).toLocaleTimeString("en-US", {
                                hour: "2-digit",
                                minute: "2-digit",
                                hour12: true,
                              })
