...
function addHeader(doc) {
  ...
  doc.image("/Users/user/Desktop/TigerTix/TigerTix/src/assets/USTLogo.png", 50, 30, { width: 70 });

  // Add the IPEA logo in the center
  doc.image("/Users/user/Desktop/TigerTix/TigerTix/src/assets/IPEA Logo.png",450, 30, { width: 50 });

  // Add the TigerTix logo on the right
  doc.image("/Users/user/Desktop/TigerTix/TigerTix/src/assets/tigertix_logo.png", 500, 50, { width: 70 });
...
exports.generateEventReport = async (req, res) => {
  try {
    ...
    if (selectedColumns.length === 0) {
      return res.status(400).json({ message: "Invalid or no columns selected." });
    }
    ...
    doc
      .fontSize(12)
      .text(
        `Generated By: ${user.first_name && user.last_name ? `${user.first_name} ${user.last_name}` : "Administrator"}`,
        { align: "left" }
      )
      .text(`Role: ${user.role || "Administrator"}`, { align: "left" })
      .text(`Date and Time Reported: ${new Date().toLocaleString()}`, { align: "left" })
      .moveDown();

    // Calculate the maximum height required for the header row
    let headerRowHeight = 20; // Default header row height
    selectedColumns.forEach((col) => {
      const headerTextHeight = doc.heightOfString(col.replace(/_/g, " ").toUpperCase(), {
    ...
    selectedColumns.forEach((col) => {
      doc
        ...
        .text(col.replace(/_/g, " ").toUpperCase(), columnPositions[col] + 5, y + 5, {
          width: columnWidth - 10, // Adjust width for padding
          align: "left",
        });
    });
    ...
    rows.forEach((row) => {
      ...
      selectedColumns.forEach((col) => {
        const value = col === "full_name" ? `${row.first_name} ${row.last_name}` : row[col] || "-";
